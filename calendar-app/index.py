...
app.layout = dbc.Container(
    [
                            ...
                            [
                                ...
                                dbc.Col(
                                    [
                                        ...
                                        html.Div(
                                            id="div-data-concatenada", hidden=True
                                        ),
                                    ],
                                    md=4,
                                    className="secundary-color",
                                    style={"border-top-right-radius": "7%"},
                                ),
                            ],
        ...
        dbc.Modal(
            [
                ...
                dbc.ModalBody(
                    [
                        ...
                        html.Div(
                            id="required-field-notification",
            ...
            style={"color": "#000000", "background-color": "rgba(255, 255, 255, 0.4)"},
            id="modal-tarefa",
            is_open=False,
        ),
    ],
...
def render_calendar_content(botao_avanca, botao_volta, mm, yy):
    ...
    for d in range(1, last_day + 1):
        ...
        day = (
            "SEG"
            if days_of_week.index(day) == 6
            else days_of_week[days_of_week.index(day) + 1]
        )
        if day == "SEG":
            c += 1
...
def update_lista_eventos(
    n_clicks, n_clicks2, data_conc, horario, titulo, local, descricao, lista_de_eventos
):
    ...
    if "submit-tarefa" in changed_id:
        if not horario:
            ...
            lista_de_eventos[data_conc].append(
                {
                    "horario": horario,
                    "titulo": titulo,
                    "local": local,
    ...
    if "delete_event" in changed_id and any(n_clicks2):
        ...
        idx = dict_id["index"]
        lista_de_eventos[data_conc] = [
            i for i in lista_de_eventos[data_conc] if i["id"] != idx
        ]
...
def update_card_geral(active_cell, lista_de_eventos, calendar_data, mes, ano):
    ...
    num_eventos = (
        0
        if data_conc not in lista_de_eventos.keys()
        else len(lista_de_eventos[data_conc])
    )

    if num_eventos != 0:
        lista_ordenada_de_eventos = sorted(
            lista_de_eventos[data_conc], key=lambda d: d["horario"]
        )
    ...
    if num_eventos == 0:
        card_tarefa = dbc.Card(
            ...
            className="mb-3",
            style={"maxWidth": "540px", "background-color": "#000000"},
            id="card-tarefa",
        )
            ...
            [
                dbc.Row(
                    [
                        dbc.Col(
                            html.H4(
                                lista_ordenada_de_eventos[i]["horario"],
                                style={
                                    "font-weight": "bold",
                                    "text-align": "center",
                                    "font-size": "16px",
                        ...
                        dbc.Col(
                            dbc.CardBody(
                                [
                                    html.H4(
                                        lista_ordenada_de_eventos[i]["titulo"],
                                        style={
                                            "font-weight": "bold",
                                            "margin-top": "-10px",
                                            "font-size": "16px",
                                    ...
                                    html.P(
                                        lista_ordenada_de_eventos[i]["local"],
                                        style={
                                            "margin-top": "-4px",
                                            "font-size": "12px",
                                            "color": "gray",
